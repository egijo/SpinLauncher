...
roi = None
tracker = None
roi_frame = None  # <<< Frame der ROI-Auswahl

start_pos = None
meter_per_pixel = None
results = []

tracking_frame_index = 0  # zählt nur Tracking-Frames
...
while True:
    ...
    # ------------------------------------------------------
        ...
        tracker = create_csrt_tracker()
        tracker.init(roi_frame, roi)   # <<< gleiches Frame!
        tracking_active = True
        paused = False
        start_pos = None
...
# DATEIEN
# ==========================================================
VIDEO_PATH = "Videos/19_40_5_undistorted.mp4"

base_name = os.path.splitext(os.path.basename(VIDEO_PATH))[0]
csv_dir   = os.path.dirname(VIDEO_PATH)
CSV_PATH  = os.path.join(csv_dir, base_name + ".csv")

CHESSBOARD_DATA_PATH = "chessboard.npz"
...
# ==========================================================
# PIXEL → METER (AUS GESPEICHERTEM SCHACHBRETT)
# ==========================================================
if not os.path.exists(CHESSBOARD_DATA_PATH):
    raise FileNotFoundError("Schachbrett-Datei nicht gefunden")

data = np.load(CHESSBOARD_DATA_PATH)

best_pattern = tuple(data["pattern"])
best_corners = data["corners"]

# mittlerer Abstand benachbarter Ecken (horizontal)
distances = []
for i in range(best_pattern[0] - 1):
    distances.append(
        np.linalg.norm(best_corners[i + 1] - best_corners[i])
    )
